...
import os
import argparse
from extract.ingest_csv_data import _get_engine, _import_sql_folder
from utils.get_config import _get_config
...
def main(args=None):
    ...
    config = _get_config(src_root + '/config/ingest_config.json')
    engine = _get_engine(config["HOST"], config["DB_NAME"])
